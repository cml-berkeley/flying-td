...
!	hm is reference height (often = initial him)
    ...
    real(8) :: emax, err, f, fneg, fpos, fsp, fsr, xf, yf, &
               ...
               fvdw_output
    real(8), dimension (:,:), allocatable :: vdwMolecularForceMap, preduced, FHsave1, FHsave2, hsave, hnewsave, recssisave
	...
	!Siddhesh-Jinglin
	!=======================
	integer :: g_if_mod_fh                  !Siddhesh !whether modify the physical clearance using probability model
	integer :: g_if_eff_area                !Siddhesh !whether use Ce to reduce air bearing force or not
	integer :: g_SBLsize                    !Siddhesh !determine size of the SBL tables
	real(8) :: g_lube_thick                 !lube thickness t
	real(8) :: g_air_diam                   !diameter of air molecules
	real(8) :: g_ys, g_sigr                 !Normalized ys, original sigma	
	real(8), dimension(:), allocatable ::  g_hstar, g_contact, g_adhesion, g_friction
	!Siddhesh-Jinglin
