...
!	hamacker consts ahc and bhc added in CMLAir 6.20
    ...
    !Siddhesh-Jingling: read SBL params
   read(2, *)
   read(2, *)
   read(2, *) g_lube_thick, g_air_diam, g_if_mod_fh, g_if_eff_area, g_SBLsize
   !Siddhesh-Jinglin   
...
!============================================
    ...
    integer :: i, j, k
    !Siddhesh-Jinglin
    real(8) :: sigmar, etarr, xyz, sigmasr, temp, alpha 
    ...
    allocate (FHsave1 (nx, ny))
    allocate (FHsave2 (nx, ny))
    allocate (hsave (nx, ny))
    allocate (hnewsave (nx, ny))
    allocate (recssisave (nx, ny))
    allocate (preduced (nx, ny))
    ...
    !Siddhesh-Jinglin
    allocate (g_hstar(g_SBLsize))
    allocate (g_contact(g_SBLsize))
    allocate (g_adhesion(g_SBLsize))
    allocate (g_friction(g_SBLsize))
    open(400,err=999, file='hstar.dat', status='old')
    read(400, *, err = 998) (g_hstar(i), i=1,g_SBLsize)
    close(400)
    open(400,err=999, file='contact.dat', status='old')
    read(400, *, err = 998) (g_contact(i), i=1,g_SBLsize)
    close(400)
    open(400,err=999, file='adhesion.dat', status='old')
    read(400, *, err = 998) (g_adhesion(i), i=1,g_SBLsize)
    close(400)
    open(400,err=999, file='friction.dat', status='old')
    read(400, *, err = 998) (g_friction(i), i=1,g_SBLsize)
    close(400)
    !hstar.dat, contact.dat, adhesion.dat, friction.dat all contain positive real arrays of length g_SBLsize
    !hstar.dat should contain clearance = d + ys in nm, gets saved in g_hstar(g_SBLsize)
    !contact.dat should contain contact pressure in Pa, gets saved in g_contact(g_SBLsize)
    !adhesion.dat should contain adhesion pressure in Pa, gets saved in g_adhesion(g_SBLsize)
    !friction.dat should contain friction pressure in Pa, gets saved in g_friction(g_SBLsize)
    
    !calculate ys
    sigmar = rsik/rasp
    etarr = cta * rasp**2.d0
    xyz = sigmar**2.d0 - 3.717e-4/etarr**2.d0
    sigmasr = dsqrt(xyz)
    temp = etarr * sigmasr
    alpha = 2412.74 * temp**2.d0 + 0.8968  !Jinglin
    g_ys = 4.d0 / dsqrt(pi*alpha)
    write(*,*)'sigmar = ',rsik
    write(*,*)'R = ',rasp
    write(*,*)'eta = ',cta
    write(*,*)'sigmas = ',sigmasr*rasp
    !write(*,*)'alpha = ',alpha
    write(*,*)'ys = ',g_ys*rsik
    g_sigr = sigmasr / sigmar
    !normalize
    g_lube_thick = g_lube_thick / rsik 
    g_air_diam = g_air_diam / rsik
    !Siddhesh-Jinglin
    
    return
    
    999	write(*,*)'Error opening SBLparams'
	write(numRes,*) 'Error opening SBLparams'
	write(numRes,*) 'errorcode: SBLopenerr'
	stop

    998	write(*,*)'Error reading SBLparams'
	write(numRes,*) 'Error reading SBLparams'
	write(numRes,*) 'errorcode: SBLreaderror'
	stop
