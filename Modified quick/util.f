...
c=================================================
!Siddhesh-Jinglin: find the contact,adhesion,friction for a specific clearance: all clearance is the same, so I only do locate once
! y1 is adhesion force, y2 is contact force, y3 is friction force, x is clerance = d + ys
      subroutine finteg_SBL(x,y1,y2,y3) 
c=================================================
      use Q4_globals
      implicit none
      ...
      call locate(g_hstar,g_SBLsize,x,j)
      
      if(g_hstar(j).eq.g_hstar(j+1)) write(*,*)'In finteg, j = ',j
      a=(x-g_hstar(j+1))/(g_hstar(j)-g_hstar(j+1))
      b=(x-g_hstar(j))/(g_hstar(j+1)-g_hstar(j))
!use dmax1 to make sure the sign is correct
      y1=dmax1(a*g_adhesion(j)+b*g_adhesion(j+1),0.d0)
      y2=dmax1(a*g_contact(j)+b*g_contact(j+1),0.d0)      
      y3=dmax1(a*g_friction(j)+b*g_friction(j+1),0.d0)
