   ...
   !Siddhesh-Jinglin
    ...
    do j=2,nym1
	    ...
	    do i=2,nxm1
	        ...
	        !Siddhesh-Jinglin: modify the clearance according to the probability model
            fpi=(bearx(ip1,j)+bearx(i,j))/4.d0*mod_fh((recssi(ip1,j)+  &
                (hnew(i,j)+hnew(ip1,j))/2.d0)*hm)/hm*deltay  
            fmi=(bearx(im1,j)+bearx(i,j))/4.d0*mod_fh((recssi(i,j) +   &
     	        (hnew(i,j)+hnew(im1,j))/2.d0)*hm)/hm*deltay
     	    fpj=(beary(i,jp1)+beary(i,j))/4.d0*mod_fh((recssj(i,jp1) + &
	            (hnew(i,j) + hnew(i,jp1))/2.d0)*hm)/hm*deltax
	        fmj=(beary(i,jm1)+beary(i,j))/4.d0*mod_fh((recssj(i,j) +   &
                (hnew(i,j)+hnew(i,jm1))/2.d0)*hm)/hm*deltax  
...
!    		idisc,mitp(1),nx,ny,nx,ny,0,GS_Iterations,negp,1)
    ...
    !====================================================
    !Siddhesh-Jinglin
    real(8) function mod_fh(fh)
    !d: physical clearance
    !====================================================
    use Q4_globals
    implicit none
    ...
    if ((g_if_mod_fh.eq.0)) then ! Siddhesh
        mod_fh = fh-g_ys*rsik
    else
	    ! normalize
	    fh_star = fh/rsik
	    ! h to hbar
	    mod_fh = dexp(-(fh_star-g_ys-g_air_diam)**2.d0/2.d0/g_sigr**2)*g_sigr/(dsqrt(2.d0*pi)) &
	        +(fh_star-g_ys)/2*(1+derf((fh_star-g_ys-g_air_diam)/dsqrt(2.d0)/g_sigr))	
	    mod_fh = rsik * mod_fh
	endif
